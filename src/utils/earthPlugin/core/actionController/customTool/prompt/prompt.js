...
class Prompt {
   ...
   * @param {Object} opt
   * @param {window.XEarth.Cartesian3 | Array} [opt.position] 弹窗坐标 （type=2时生效）
   * @param {Boolean} opt.show 是否显示
   * @param {Function} [opt.success] 创建成功的回调函数
   * @param {Number} [opt.type=1] 1~位置变化提示框 / 2~固定坐标提示框
   * @param {window.XEarth.Cartesian3 | Array} opt.position 固定坐标提示框的坐标（ cartesian3 / [101,30] ），type为1时，可不设置此参数
  ...
  constructor(viewer, opt) {
    ...
    if (this.opt.anchor) {
      anchorHtml += `
            <div class="prompt-anchor-container">
                <div class="prompt-anchor" style="background:${background} ">
                </div>
            </div>
            `
    }
    ...
    let contentText = ``
    if (this.opt.content.length > 0) {
      this.opt.content.forEach((element) => {
        contentText += ` <div style="display: flex;"><div style="color:yellow">${element.type}：</div>
  ...
  bindRender() {
    ...
    this.rendHandler = this.viewer.scene.postRender.addEventListener(
      function () {
        ...
        let entity = this.viewer.entities.getById(this.opt.entityId)
        if (!entity) {
          let ds = this.viewer.dataSources.getByName(this.opt.datasourceName)
          if (ds && ds.length > 0) {
            entity = ds[0].entities.getById(this.opt.entityId)
          }
        }
        if (!entity) {
          that.promptDom.style.display = 'none'
        ...
        that.position = entity.position.getValue(this.viewer.clock.currentTime)
        if (!that.position) return
        if (that.position instanceof window.XEarth.Cartesian3) {
          let px = window.XEarth.SceneTransforms.wgs84ToWindowCoordinates(
            that.viewer.scene,
            that.position
          )
          if (!px) return
          const occluder = new window.XEarth.EllipsoidalOccluder(
  ...
  /**
   *
   * @param {window.XEarth.Cartesian3 | Object} px 弹窗坐标
   * @param {String} html 弹窗内容
   */
  update(px, html) {
    if (px instanceof window.XEarth.Cartesian3) {
      this.position = px.clone()
      px = window.XEarth.SceneTransforms.wgs84ToWindowCoordinates(
        this.viewer.scene,
        px
      )
    }
  ...
  isInView() {
    ...
    let px = null
    if (this.position instanceof window.XEarth.Cartesian2) {
      px = this.position
    } else {
      px = window.XEarth.SceneTransforms.wgs84ToWindowCoordinates(
        this.viewer.scene,
  ...
  transPosition(p) {
    let position
    if (Array.isArray(p)) {
      const posi = window.XEarth.Cartesian3.fromDegrees(p[0], p[1], p[2] || 0)
      position = posi.clone()
    } else if (p instanceof window.XEarth.Cartesian3) {
      position = p.clone()
    } else {
      // 像素类型
      position = p
