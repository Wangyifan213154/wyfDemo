class CreateFrustumPure {
  constructor(options) {
    ...
    this.lineColor = options.lineColor || new window.XEarth.Color(0.0, 1.0, 1.0, 1.0)
        ...
        material: new window.XEarth.Material({
          fabric: {
            ...
            uniforms: {
              ...
              time2: 0,
              mixColor: lineColor, //new window.XEarth.Color(0.0, 1.0, 0.0, 1.0),
              mixRatio: 0.0,
            ...
            czm_material czm_getMaterial(czm_materialInput materialInput)
              {
                  ...
                  // float alpha = smoothstep(0.0, 1.0, pow(fragColor.r,2.0));
                  // float alphaPulse = smoothstep(0.0, 1.0, pow(fragColor2.r,2.0));
                  // float mixAlpha = mix(alpha,alphaPulse,0.1); // 无背景色
  ...
  addOutline() {
    ...
    let instance = new window.XEarth.GeometryInstance({
      geometry: geometry,
      attributes: {
        color: window.XEarth.ColorGeometryInstanceAttribute.fromColor(this.lineColor)
      }
    })
    let primitive = new window.XEarth.Primitive({
      geometryInstances: instance,
