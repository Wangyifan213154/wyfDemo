...
onMounted(() => {
  ...
  // bottom动态组件选中阶段性描述 显示阶段性描述
  // emitter.on('bottomComp', (val) => {
  //   if (val.name == 'bottomStageDescription') {
  //     let param = {
  //       name: '阶段性描述',
  //       tag: 'stageDescription',
  //       actived: val.name == 'bottomStageDescription' ? false : true,
  //       sign: false,
...
const state = reactive({
  ...
  navbarBtn: [
    ...
    // {
    //   name: '星座配置',
    //   tag: 'constellationConfiguration',
    //   actived: false,
    //   sign: false,
...
const selectMenu = (item) => {
  ...
  switch (item.tag) {
    ...
    case 'ceshi':
      ...
      if (state.ceshi) {
        // 图层添加节点
        // layerManagementSource.addNode({ name: 'hahah', code: 'hahah' },'蓝方')
        // 右上角提示
        // beautyToast.success({
        //   title: 'Success',
        ...
        // 图层删除节点
        // layerManagementSource.deleteNode('hahah')
        // 右上角提示
        // beautyToast.error({
        //   title: 'Error',
