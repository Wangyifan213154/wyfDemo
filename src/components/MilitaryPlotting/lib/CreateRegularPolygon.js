...
const CreateRegularPolygon = function (viewer, resultList, options, callback) {
  ...
  handler.setInputAction(function (event) {
    toolTip = '右键结束绘制'
    if (window.XEarth.defined(pegularPolygon)) {
      return
    }
  ...
  handler.setInputAction(function (movement) {
    ...
    if (window.XEarth.defined(pegularPolygon)) {
      ...
      let distance = window.XEarth.Cartesian3.distance(
        new window.XEarth.Cartesian3.fromDegrees(
          centerP.lng,
          centerP.lat,
          0
        ),
...
function calculatePoints(centerPoint, distance, num) {
  let ellipse = new window.XEarth.EllipseOutlineGeometry({
    center: centerPoint,
    semiMajorAxis: distance,
    semiMinorAxis: distance,
    granularity: 0.0001 //0~1 圆的弧度角,该值非常重要,默认值0.02,如果绘制性能下降，适当调高该值可以提高性能
  })
  let geometry = new window.XEarth.EllipseOutlineGeometry.createGeometry(
    ellipse
  )
  let circlePoints = []
  ...
  for (let i = 0; i < posNum; i++) {
    let curPos = new window.XEarth.Cartesian3(
      values[i * 3],
      values[i * 3 + 1],
      values[i * 3 + 2]
    )
