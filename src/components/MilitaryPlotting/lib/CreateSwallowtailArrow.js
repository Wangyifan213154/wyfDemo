 ...
 * 创建燕尾箭头
 * @Author: Wang jianLei
 * @Date: 2022-04-14 14:49:29
 * @Last Modified by: Wang JianLei
 * @Last Modified time: 2022-04-15 18:21:01
 */
    ...
    if (anchorpoints.length > 1) {
      if (!window.XEarth.defined(polygon)) {
        ...
        SwallowtailArrowPoints =
          window.XEarth.Cartesian3.fromDegreesArrayHeights(GeoPoints)
        polygon.polygon.hierarchy = new window.XEarth.CallbackProperty(
          function () {
            return new window.XEarth.PolygonHierarchy(SwallowtailArrowPoints)
          },
          false
        )
        ...
        SwallowtailArrowPoints =
          window.XEarth.Cartesian3.fromDegreesArrayHeights(GeoPoints1)
      }
    } else {
      return
    }
...
function plotingSwallowtailArrow(viewer, anchorpoints, color) {
  ...
  let polygon = viewer.entities.add({
    ...
    polygon: {
      hierarchy: window.XEarth.Cartesian3.fromDegreesArrayHeights(GeoPoints),
      material: color
    }
  })
  return polygon
