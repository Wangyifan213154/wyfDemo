 ...
 * 创建集结地
 * @Author: Wang jianLei
 * @Date: 2022-04-15 23:16:38
 * @Last Modified by: Wang JianLei
 * @Last Modified time: 2022-04-15 23:49:23
 */
    ...
    if (anchorpoints.length > 1) {
      ...
      if (!window.XEarth.defined(polygon)) {
        ...
        polygon.polygon.hierarchy = new window.XEarth.CallbackProperty(
          function () {
            return new window.XEarth.PolygonHierarchy(
              window.XEarth.Cartesian3.fromDegreesArrayHeights(jjdPoints)
            )
          },
          false
        )
