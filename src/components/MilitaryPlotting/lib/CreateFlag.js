...
const CreateFlag = function (viewer, resultList, options, callBack) {
  ...
  handler.setInputAction(function (evt) {
    ...
    if (!window.XEarth.defined(flagEntity)) {
      flagEntity = viewer.entities.add({
        id: id,
        name: flagType[type],
        polygon: {
          hierarchy: new window.XEarth.CallbackProperty(function () {
            let arrow = []
            let res = fineArrow()
            for (let i = 0; i < res.length; i++) {
              let cart3 = new window.XEarth.Cartesian3(
                res[i].x,
                res[i].y,
                res[i].z
              )
            ...
            return new window.XEarth.PolygonHierarchy(arrow)
          }, false),
  ...
  function fineArrow() {
    ...
    if (points < 2) {
      ...
      return window.XEarth.Cartesian3.fromDegreesArray(
        [].concat.apply([], arcArr)
      )
    }
  }
