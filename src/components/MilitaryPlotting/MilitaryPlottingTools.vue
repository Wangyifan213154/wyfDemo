...
export default {
  ...
  mounted() {
    my_MilitaryPlotting = new MilitaryPlotting(window.EarthViewer)
  },
  methods: {
    draw(type) {
    ...
    createRegularPolygon(num) {
      my_MilitaryPlotting.CreateRegularPolygon(
        {
          color: window.XEarth.Color.fromCssColorString(this.color),
          id: this.setSessionid(),
          num: num
        },
        function (el) {
          console.log('在这里执行多边形绘制后回调方法', el, my_MilitaryPlotting)
        }
      )
    },
    //创建多边形
    ...
    createFreePolygon() {
      my_MilitaryPlotting.CreateFreePolygon(
        ...
        function (el) {
          console.log('在这里执行自由面绘制后回调方法', el, my_MilitaryPlotting)
        }
      )
    },
    //创建圆滑曲线
    ...
    createFreeLine() {
      my_MilitaryPlotting.CreateFreeLine(
        ...
        function (el) {
          console.log('在这里执行自由线绘制后回调方法', el, my_MilitaryPlotting)
        }
      )
    },
    //创建曲线旗标
